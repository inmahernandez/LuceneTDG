...
public final class LevensteinDistance implements StringDistance {
    ...
    public float getDistance (String target, String other) {
      ...
      //TODO: Check the actual necessary size of this array
        ops = new DamerauLevenshteinOp[Math.max(n, m) + 1];
        ...
        for (j = 1; j<=m; j++) {
            ...
            for (i=1; i<=n; i++) {
                ...
                calculateMinCostOp(d, ops, p, i, cost);
            }
  ...
  private void calculateMinCostOp(int[] d, DamerauLevenshteinOp[] ops, int[] p, int i, int cost) {
	  if(d[i-1]+1 < p[i]+1) {
		  d[i] = d[i-1] + 1;
	  }
