 ...
 *
 */
public class SpellCheckerTDG implements java.io.Closeable {
   ...
   */
  public SpellCheckerTDG(Directory spellIndex, StringDistance sd) throws IOException {
   ...
   */
  public SpellCheckerTDG(Directory spellIndex) throws IOException {
   ...
   */
  public SpellCheckerTDG(Directory spellIndex, StringDistance sd, Comparator<SuggestWord> comparator) throws IOException {
   ...
   */
  public List<SuggestionTDG> suggestSimilar(String word, int numSug) throws IOException {
   ...
   * @see #suggestSimilar(String, int, IndexReader, String, SuggestMode, float)
   */
  public List<SuggestionTDG> suggestSimilar(String word, int numSug, float accuracy) throws IOException {
   ...
   * 
   */
  public List<SuggestionTDG> suggestSimilar(String word, int numSug, IndexReader ir,
  ...
  public List<SuggestionTDG> suggestSimilar(String word, int numSug, IndexReader ir,
    ...
    // obtainSearcher calls ensureOpen
	  List<SuggestionTDG> ret = new ArrayList<SuggestionTDG>();
    ...
    try {
      ...
      if (suggestMode==SuggestMode.SUGGEST_WHEN_NOT_IN_INDEX && freq > 0) {
    	ret.add(new SuggestionTDG(word, Float.MAX_VALUE, new ArrayList<DamerauLevenshteinOp>()));
        return ret;
      }
      ...
      //CAREFUL: prefix length must be 0, otherwise an exact search is performed 
      Query query = new FuzzyQuery(term, LevenshteinAutomata.MAXIMUM_SUPPORTED_DISTANCE, 0,
    		  word.length(), false);
  ...
  //    System.out.println("HITS: " + hits.length());
      ...
      Queue<List<DamerauLevenshteinOp>> opQueue = new LinkedList<List<DamerauLevenshteinOp>>();
      ...
      for (int i = 0; i < stop; i++) {
        ...
        sugWord.string = indexSearcher.doc(hits[i].doc).get(field); // get orig word

        // modification: for us, returning the same string is ok, maximum score then
        ...
        List<DamerauLevenshteinOp> ops = ((DamerauLevenshteinDistanceTDG) sd).getMinCostEditionOps();
      ...
      for (int i = sugQueue.size() - 1; i >= 0; i--) {
        SuggestWord sug = sugQueue.pop();
        List<DamerauLevenshteinOp> ops = opQueue.poll();
		ret.add(new SuggestionTDG(sug.string, sug.score, ops));
      }

      //TODO: Order by score, in ascending order
   ...
   * @throws AlreadyClosedException if the {@link SpellCheckerTDG} is already closed
   */
  @Override
  ...
  /**
   * Returns <code>true</code> if and only if the {@link SpellCheckerTDG} is
   * closed, otherwise <code>false</code>.
   * 
   * @return <code>true</code> if and only if the {@link SpellCheckerTDG} is
   *         closed, otherwise <code>false</code>.
   */
  boolean isClosed(){
    return closed;
